...
| `canvas_demo` | `TwgRasterCanvas` drawing (rects, rounded rects, circles/ellipses, lines, a polygon) into an **shm** window buffer. | a Wayland compositor |
...
| `gl_widget_demo` | The **widget toolkit**: real controls (button, checkbox, radio, slider, label, panel), a theme seeded from the running desktop's colour scheme, accent and UI font, box layouts with weights and no hard-coded coordinates, damage-driven partial repaint, and the input router's hover/press/click/focus/Tab/drag-capture. Two text fields (one masked) show caret, selection, drag- and double-click-select, word moves, key repeat and Ctrl+A/C/X/V against the real clipboard. The scrolling list shows the gesture handshake: **click** an item and it fires, **drag** one and the pan recogniser claims the sequence, the button is cancelled instead of clicked, and the list scrolls then coasts. Runs on the **software** canvas by default; pass `--gl` to swap in the GPU presenter (dmabuf, 2x AA) — the same tree, unchanged. | a Wayland compositor and a TrueType font; `--gl` also needs `zwp_linux_dmabuf_v1` and `EGL_MESA_image_dma_buf_export` |
| `gl_canvas_demo` | `TwgGLCanvas` — the **GPU** canvas. Anti-aliased fills and strokes, the transform stack, clipping, blend modes, scaled/tinted `IwgSurface` blits, bezier paths and FreeType text, presented zero-copy as a dmabuf. Left-drag moves, right-click quits. | `zwp_linux_dmabuf_v1`, a GL 3.3 core driver with `EGL_MESA_image_dma_buf_export`, and a TrueType font |
