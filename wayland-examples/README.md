...
| `gl_widget_demo` | The **widget toolkit**: real controls (button, checkbox, radio, slider, label, panel), a theme seeded from the running desktop's colour scheme, accent and UI font, box layouts with weights and no hard-coded coordinates, damage-driven partial repaint, and the input router's hover/press/click/focus/Tab/drag-capture. Runs on the **software** canvas — the `gl_` prefix only tells the Makefile to link freetype/fontconfig. | a Wayland compositor and a TrueType font |
