...
| `gl_widget_demo` | The **widget toolkit**: real controls (button, checkbox, radio, slider, label, panel), a theme seeded from the running desktop's colour scheme, accent and UI font, box layouts with weights and no hard-coded coordinates, damage-driven partial repaint, and the input router's hover/press/click/focus/Tab/drag-capture. Runs on the **software** canvas by default; pass `--gl` to swap in the GPU presenter (dmabuf, 2x AA) — the same tree, unchanged. | a Wayland compositor and a TrueType font; `--gl` also needs `zwp_linux_dmabuf_v1` and `EGL_MESA_image_dma_buf_export` |
