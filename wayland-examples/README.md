...
| `gl_widget_demo` | The **widget tree** in a live window: nested widgets, child clipping, damage-driven partial repaint (the moving box invalidates only itself), per-buffer damage under double buffering, and fontconfig-resolved text. Runs on the **software** canvas — the `gl_` prefix only tells the Makefile to link freetype/fontconfig. | a Wayland compositor and a TrueType font |
