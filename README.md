...
| `wayland-client/widgets/` | Retained widget toolkit on `TwgCanvas`: tree, per-buffer damage, input routing (multi-pointer, touch-ready), box/grid/anchor layouts, theming from `desktop_theme`, and controls. RTL-only, so the software path never drags in a C library. ([design](docs/wayland-widgets-design.md), [status](docs/wayland-widgets-status.md), [roadmap](docs/wayland-widgets-roadmap.md).) | rt + tiers + classes | **pasbuild** |
| `wayland-client/widgets-gl/` | Bridge: `TwgGLPresenter`, which renders a widget window on the GPU and presents it as a dmabuf. Its own module so neither `gl` nor `widgets` depends on the other. | widgets + gl + tiers | **pasbuild** |
| `wayland-client/widgets-xkb/` | Bridge: `TwgXkbTranslator`, which turns the raw evdev codes and XKB keymap fd `wl_keyboard` delivers into keysyms and UTF-8 text (with compose/dead keys) — what a text field needs before it can be typed into. Its own module because that needs `libxkbcommon` and `widgets` is RTL-only. | widgets + classes | **pasbuild** |
