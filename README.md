...
| `wayland-client/rt/` | Client runtime: generated `wayland.pas` (core protocol proxies) + the RTL-only utilities `wayland_canvas` ([docs](docs/wayland-canvas.md)), `wayland_surface` / `wayland_accel_canvas` ([docs](docs/wayland-accel-canvas.md)) and `wayland_dmabuf`. | `wayland-common` (unit path) | **pasbuild** |
...
| `wayland-client/gl/` | GPU canvas backend: surfaceless EGL + GL 3.3 core context, dmabuf-exporting FBO render targets, a FreeType glyph atlas, and `TWaylandGLCanvas` — the OpenGL implementation of `TWaylandAccelCanvas`. ([docs](docs/wayland-accel-canvas.md).) The one module that is **not** RTL-only (links `libEGL`/`libGL`/`libfreetype`), so it is excluded from the default build and nothing else depends on it. | `wayland-client/rt` | **pasbuild** |
