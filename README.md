...
| `wayland-client/widgets/` | Retained widget toolkit on `TwgCanvas`: tree, per-buffer damage, input routing (multi-pointer, touch-ready), box/grid/anchor layouts, theming from `desktop_theme`, and controls. RTL-only, so the software path never drags in a C library. ([design](docs/wayland-widgets-design.md).) | rt + tiers + classes | **pasbuild** |
